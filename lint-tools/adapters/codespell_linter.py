...
# Dictionary path - check multiple locations
DICTIONARY = REPO_ROOT / "lint-config" / "dictionary.txt"
if not DICTIONARY.exists():
    DICTIONARY = REPO_ROOT / "lint-tools" / "adapters" / ".." / ".." / "lint-config" / "dictionary.txt"
...
def run_codespell(path: Path, dictionary: Path | None) -> str:
    cmd = [
        sys.executable,
        "-m",
        "codespell_lib",
    ...
    if dictionary and dictionary.exists():
        cmd.extend(["--ignore-words", str(dictionary)])
    try:
...
def check_file(filename: str) -> list[LintMessage]:
    ...
    try:
        run_codespell(path, DICTIONARY if DICTIONARY.exists() else None)
...
def check_dictionary(filename: str) -> list[LintMessage]:
    ...
    path = Path(filename).absolute()
    if not path.exists():
        return []  # Dictionary file not required
    try:
