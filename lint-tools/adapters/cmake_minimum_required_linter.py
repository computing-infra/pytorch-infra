...
# ///
"""
CMake minimum version linter for Ascend/pytorch.

Adapted from PyTorch's cmake_minimum_required_linter.py.
Removed dependency on PyTorch's tools.setup_helpers.env module.
Hardcoded CMAKE_MINIMUM_VERSION for Ascend/pytorch (3.18).
"""
...
# Ascend/pytorch uses cmake_minimum_required(VERSION 3.18)
CMAKE_MINIMUM_VERSION_STRING = "3.18"
...
def check_cmake(path: Path, repo_root: Path) -> list[LintMessage]:
    with path.open(encoding="utf-8") as f:
        for i, line in enumerate(f, start=1):
            if match := CMAKE_MINIMUM_REQUIRED_PATTERN.search(line):
                version = match.group("version")
                # Only check root CMakeLists.txt for exact version match
...
def check_file(filename: str, repo_root: Path) -> list[LintMessage]:
    path = Path(filename).absolute()
    basename = path.name.lower()
    if basename in ("cmakelists.txt", "cmakelists.txt.in") or basename.endswith(
        (".cmake", ".cmake.in")
    ):
        return check_cmake(path, repo_root)
...
def main() -> None:
    ...
    with concurrent.futures.ProcessPoolExecutor(
        ...
        futures = {executor.submit(check_file, x, repo_root): x for x in args.filenames}
